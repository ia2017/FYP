...
#            konfig['CONV_FILENAME'] = 'config_CFD'
    ...
    # MICHALIS_ADDITIONS
    if os.path.exists("config_CFD.csv"):
        from_file = open("config_CFD.csv", mode="r")
        line = from_file.readline()
        str2 = ''
        count_pseudo = 0
        for count in range(0, len(line)):
            if (line[count_pseudo] == '[' and line[count_pseudo + 1] == '0' and line[count_pseudo + 2] == ']'):
                count_pseudo += 3
            else:
                str2 += line[count_pseudo]
                count_pseudo += 1
                if count_pseudo >= len(line):
                    break
        to_file = open("history_direct.csv", mode="w")
        to_file.write(str2)
        shutil.copyfileobj(from_file, to_file)
        to_file.close()
        from_file.close()
    # END OF MICHALIS ADDITIONS
