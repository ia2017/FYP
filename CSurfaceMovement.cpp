...
vector<vector<su2double> > CSurfaceMovement::SetSurface_Deformation(CGeometry *geometry, CConfig *config) {
  ...
  if ((config->GetDesign_Variable(0) == FFD_CONTROL_POINT_2D) ||
      ...
      (config->GetDesign_Variable(0) == FFD_CONTROL_POINT_NORMAL) ||
      (config->GetDesign_Variable(0) == FFD_NACELLE) ||
      (config->GetDesign_Variable(0) == FFD_GULL) || 
      (config->GetDesign_Variable(0) == FFD_GULL_NEW) ||
      (config->GetDesign_Variable(0) == FFD_GULL_SIMPLE) || 
      (config->GetDesign_Variable(0) == FFD_GULL_NEW_IND) ||
      (config->GetDesign_Variable(0) == FFD_GULL_ROT) ||
      (config->GetDesign_Variable(0) == FFD_GULL_ROT_IND) ||
      ...
      (config->GetDesign_Variable(0) == FFD_SWEEP) ||
    ...
    if (config->GetFFDTag(iDV)== FFDBox->GetTag()){
      switch ( config->GetDesign_Variable(iDV) ) {
        ...
        case FFD_CAMBER_2D :  case FFD_THICKNESS_2D :
          ...
          break;
        case FFD_CONTROL_POINT : case FFD_CONTROL_POINT_NORMAL :  case FFD_NACELLE :
          iIndex = SU2_TYPE::Int(fabs(config->GetParamDV(iDV, 1)));
          jIndex= SU2_TYPE::Int(fabs(config->GetParamDV(iDV, 2)));
          kIndex = SU2_TYPE::Int(fabs(config->GetParamDV(iDV, 3)));
          if ((iIndex > lDegree) || (jIndex > mDegree) || (kIndex > nDegree)) OutOffLimits = true;
          break;
        case FFD_GULL: case FFD_TWIST: case FFD_SWEEP: case FFD_TAPER: case FFD_GULL_NEW: case FFD_GULL_SIMPLE: case FFD_GULL_NEW_IND: case FFD_GULL_ROT: case FFD_GULL_ROT_IND:
...
void CSurfaceMovement::ApplyDesignVariables(CGeometry *geometry, CConfig *config, CFreeFormDefBox **FFDBox, unsigned short iFFDBox) {
  ...
  for (iDV = 0; iDV < config->GetnDV(); iDV++) {
    switch ( config->GetDesign_Variable(iDV) ) {
      ...
      case FFD_CONTROL_POINT_NORMAL :  SetFFDCPChangeNormal(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
      ...
      case FFD_GULL_NEW:          SetFFDGullNew(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
      case FFD_GULL_SIMPLE:       SetFFDGullSimple(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
      case FFD_GULL_NEW_IND:      SetFFDGullNewInd(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
      case FFD_GULL_ROT:          SetFFDGullRot(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
      case FFD_GULL_ROT_IND:      SetFFDGullRotInd(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
      ...
      case FFD_SWEEP:             SetFFDSweep(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
      case FFD_TAPER:             SetFFDTaper(geometry, config, FFDBox[iFFDBox], FFDBox, iDV, false); break;
    }
  }
}
...
bool CSurfaceMovement::SetFFDCPChangeNormal(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
  ...
  if (design_FFDBox.compare(FFDBox->GetTag()) == 0) {
    ...
    unsigned short jind = SU2_TYPE::Int(config->GetParamDV(iDV, 2));

    su2double* bottom = FFDBox->GetCoordControlPoints(0, jind, 0);
    su2double* top = FFDBox->GetCoordControlPoints(0, jind, FFDBox->GetnOrder() - 1);

    /*--- Calculating normal direction vector ---*/

    su2double normi = top[0] - bottom[0];
    su2double normj = top[1] - bottom[1];
    su2double normk = top[2] - bottom[2];

    su2double norm = sqrt(normi * normi + normj * normj + normk * normk);
    su2double factor = 1.0 / norm;

    if (config->GetnDV_Value(iDV) == 1) {
      Ampl = config->GetDV_Value(iDV) * Scale;

      movement[0] = normi * factor * Ampl;
      movement[1] = normj * factor * Ampl;
      movement[2] = normk * factor * Ampl;
...
bool CSurfaceMovement::SetFFDGullSimple(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
                                        CFreeFormDefBox** ResetFFDBox, unsigned short iDV, bool ResetDef) const {
  su2double movement[3] = {0.0, 0.0, 0.0}, Ampl, Ampl_total, Ampl_diff, Ampl2, orig, r, FFDspan, FFDheight;
  unsigned short index[3], i, j, k, nk, nj, iPlane, iFFDBox;
  string design_FFDBox;
  su2double Scale = config->GetOpt_RelaxFactor();

  /* --- Getting Total number of j points ---*/

  nj = SU2_TYPE::Int(config->GetParamDV(iDV, 1)) + 1;
  nk = FFDBox->GetnOrder();

  /* --- Need to get FFD box dimension ---- */

  FFDspan = 0.3;    // Temporary ---- need to get auto
  FFDheight = 0.2;  // Temporary ---- need to get auto
  orig = FFDspan / (nj - 1);
  r = FFDheight / (nk - 1) / 2;
  ...
  //if (design_FFDBox.compare(FFDBox->GetTag()) == 0) {
    ...
    Ampl_total = config->GetDV_Value(iDV) * Scale;
    Ampl_diff = Ampl_total / (nj);
    ...
    for (j = 0; j < nj; j++) {
      ...
      /*------- Span wise deformation based on parabolic blending function ------*/
      Ampl = Ampl + Ampl_diff;
      Ampl2 = Ampl * Ampl;
    }
    /*
...
bool CSurfaceMovement::SetFFDGullNew(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
                                     CFreeFormDefBox** ResetFFDBox, unsigned short iDV, bool ResetDef) const {
  su2double movement[3] = {0.0, 0.0, 0.0}, Ampl, Ampl_total, Ampl_diff, Ampl2, orig, r, FFDspan, FFDheight, gAmpl, theta;
  unsigned short index[3], i, j, k, nk, nj, iPlane, iFFDBox;
  string design_FFDBox;
  su2double Scale = config->GetOpt_RelaxFactor();
    ...
    for (j = 0; j < nj; j++) {
      ...
      for (i = 0; i < FFDBox->GetlOrder(); i++) {
        ...
        for (k = 0; k < FFDBox->GetnOrder(); k++) {
          ...
          /* ---- Triangular approximate for horizontal movement------*/
          /*
          if (Ampl2 * Ampl2 <= orig * orig) {
            movement[1] = -(orig - sqrt(orig * orig - Ampl2 * Ampl2));
          } else {
            movement[1] = -orig;
          }*/
          ...
          if (k == 0) {
            movement[1] = r * sin(theta);
            movement[2] = movement[2] + (r - r * cos(theta));
          } else {
            movement[1] = -r * sin(theta);
            movement[2] = movement[2] - (r - r * cos(theta));
          }
...
bool CSurfaceMovement::SetFFDGullNewInd(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
                                        CFreeFormDefBox** ResetFFDBox, unsigned short iDV, bool ResetDef) const {
  su2double movement[3] = {0.0, 0.0, 0.0}, Ampl, Ampl_total, Ampl_diff, Ampl2, orig, r, FFDspan, FFDheight, gAmpl, theta;
  unsigned short index[3], i, j, k, nk, nj, iPlane, iFFDBox;
  string design_FFDBox;
...
bool CSurfaceMovement::SetFFDGullRot(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
                                     CFreeFormDefBox** ResetFFDBox, unsigned short iDV, bool ResetDef) const {
  unsigned short iOrder, jOrder, kOrder;
  su2double movement[3] = {0.0, 0.0, 0.0}, x, y, z, movement_total[3] = {0.0, 0.0, 0.0};
  unsigned short index[3], iFFDBox;
  string design_FFDBox;
  su2double Scale = config->GetOpt_RelaxFactor();

  su2double nj = FFDBox->GetmOrder();
  ...
  if (design_FFDBox.compare(FFDBox->GetTag()) == 0) {
    ...
    su2double u = config->GetParamDV(iDV, 4) - a;
    su2double v = config->GetParamDV(iDV, 5) - b;
    su2double w = config->GetParamDV(iDV, 6) - c;

    /*--- The angle of rotation. ---*/

    su2double theta_total = config->GetDV_Value(iDV) * Scale * PI_NUMBER / 180.0;
    su2double theta_diff = theta_total / (nj - 1);
...
bool CSurfaceMovement::SetFFDGullRotInd(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
                                        CFreeFormDefBox** ResetFFDBox, unsigned short iDV, bool ResetDef) const {
  unsigned short iOrder, jOrder, kOrder;
  su2double movement[3] = {0.0, 0.0, 0.0}, x, y, z, movement_total[3] = {0.0, 0.0, 0.0};
  unsigned short index[3], iFFDBox;
  ...
  if (design_FFDBox.compare(FFDBox->GetTag()) == 0) {
    ...
    su2double* le = FFDBox->GetCoordControlPoints(0, iDV, 0);
    su2double* te = FFDBox->GetCoordControlPoints(FFDBox->GetlOrder() - 1, iDV, 0);
    ...
    for (iOrder = 0; iOrder < FFDBox->GetlOrder(); iOrder++) {
      for (kOrder = 0; kOrder < FFDBox->GetnOrder(); kOrder++) {
        index[0] = iOrder;
        index[2] = kOrder;
        su2double* coord = FFDBox->GetCoordControlPoints(iOrder, iDV, kOrder);
        x = coord[0];
        y = coord[1];
        z = coord[2];
        movement[0] = a * (v2 + w2) + u * (-b * v - c * w + u * x + v * y + w * z) +
...
bool CSurfaceMovement::SetFFDSweep(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
                                   CFreeFormDefBox** ResetFFDBox, unsigned short iDV, bool ResetDef) const {
  su2double movement[3] = {0.0, 0.0, 0.0}, Ampl, Ampl_total, Ampl_diff;
  unsigned short index[3], i, j, k, nj, iPlane, iFFDBox;
  string design_FFDBox;
  su2double Scale = config->GetOpt_RelaxFactor();

  /* --- Getting Total number of j points ---*/

  nj = SU2_TYPE::Int(config->GetParamDV(iDV, 1));
...
bool CSurfaceMovement::SetFFDTaper(CGeometry* geometry, CConfig* config, CFreeFormDefBox* FFDBox,
                                   CFreeFormDefBox** ResetFFDBox, unsigned short iDV, bool ResetDef) const {
  su2double movement[3] = {0.0, 0.0, 0.0}, Ampl, Ampl_total, Ampl_diff, Ampl_x, Ampl_diffx, tc, Ampl_z, Ampl_diffz;
  unsigned short index[3], i, j, k, nx, nz, xmid, zmid, nj, iPlane, iFFDBox;
  string design_FFDBox;
  su2double Scale = config->GetOpt_RelaxFactor();
  ...
  // end = FFDBox->GetCoordCornerPoints();

  /* ---- Getting t/c ratio ------ */
  unsigned short jind = SU2_TYPE::Int(config->GetParamDV(iDV, 2));
  su2double* bottomright = FFDBox->GetCoordControlPoints(0, jind, 0);
  su2double* bottomleft = FFDBox->GetCoordControlPoints(FFDBox->GetlOrder() - 1, jind, 0);
  su2double* topleft = FFDBox->GetCoordControlPoints(FFDBox->GetlOrder() - 1, jind, FFDBox->GetnOrder() - 1);
  su2double* topright = FFDBox->GetCoordControlPoints(0, jind, FFDBox->GetnOrder() - 1);
  tc = (topright[2] - bottomright[2])/(topleft[0] - topright[0]) ;
